...
import { VideoCameraOutlined, ClockCircleOutlined, ScheduleOutlined, UserOutlined, MedicineBoxOutlined, CalendarOutlined } from '@ant-design/icons';
...
const DoctorVideoConsultations = () => {
  ...
  const getVideoConsultations = async () => {
    try {
      ...
      const response = await axios.get(
        getApiUrl(API_ENDPOINTS.DOCTOR.GET_VIDEO_CONSULTATIONS),
        {
          headers: {
  ...
  const joinVideoConsultation = async () => {
    ...
    try {
      ...
      // Update join status
      await axios.post(
        getApiUrl(API_ENDPOINTS.DOCTOR.UPDATE_VIDEO_JOIN_STATUS),
        {
  ...
  return (
    <Layout>
      <div className="container mx-auto px-4 py-8">
        ...
        <Alert
          message="Managing Video Consultations"
          description={
            <>
              <p>Provide high-quality care to your patients through video consultations:</p>
              <ol className="list-decimal ml-5 mt-2">
                <li>Join on time or a few minutes early for your scheduled consultations</li>
                <li>Ensure you have access to the patient's medical history and records</li>
                <li>Use a professional setting with good lighting and minimal background noise</li>
                ...
                >
                  <div className="flex items-start mb-4">
                    <Avatar 
                      size={64} 
                      icon={<UserOutlined />}
                      src={consultation.userInfo.profilePicture}
                      className="mr-4"
                    />
                    <div>
                      <Text className="text-lg font-semibold block">{consultation.userInfo.name}</Text>
                      <Tag color={getStatusColor(consultation.joinStatus)} className="mt-1">
                        {getStatusText(consultation.joinStatus)}
                      </Tag>
                      {consultation.joinStatus !== 'ended' && consultation.videoConsultation.joinedByPatient && (
      ...
      >
        {selectedConsultation && (
          <>
            ...
            <p className="font-semibold mb-2">{selectedConsultation.userInfo.name}</p>
            <p className="mb-4">{selectedConsultation.formattedDate} at {selectedConsultation.formattedTime}</p>
            
            <div className="mb-4 p-3 bg-gray-50 rounded-md">
              <h4 className="font-medium flex items-center"><MedicineBoxOutlined className="mr-2" /> Patient Information</h4>
              <p className="mb-1"><strong>Reason:</strong> {selectedConsultation.reason}</p>
              {selectedConsultation.symptoms && (
                <p className="mb-1"><strong>Symptoms:</strong> {selectedConsultation.symptoms}</p>
              )}
              {selectedConsultation.medicalHistory && (
                <p className="mb-1"><strong>Medical History:</strong> {selectedConsultation.medicalHistory}</p>
              )}
              {selectedConsultation.additionalNotes && (
                <p className="mb-1"><strong>Additional Notes:</strong> {selectedConsultation.additionalNotes}</p>
              )}
            </div>
            ...
            <Alert
              ...
              description={
                <ul className="list-disc ml-5">
                  <li>Ensure you're in a professional, well-lit setting</li>
                  <li>Have the patient's medical records ready for reference</li>
                  <li>Test your camera and microphone before entering the call</li>
                  <li>After the consultation, update the patient's medical records</li>
                </ul>
              }
              type="info"
              showIcon
            ...
            {selectedConsultation.videoConsultation.joinedByPatient ? (
              <Alert message="The patient has already joined the call." type="success" showIcon />
            ) : (
              <Alert message="The patient hasn't joined the call yet." type="warning" showIcon />
            )}
          </>
        )}
      </Modal>
